...
public class Tests {
    ...
    void iteratorTest(){
        ...
        iter.remove();
        iter.remove();
        //не понял почему почему должна удаляться 4, если мы переходим на следующий, т.е. на 2
        //следующий этот тот что слева???
    ...
    void sortedTest(){
        MyLinkedList<Integer> list = new MyLinkedList<>();
//        list.add(0);
        //не проходит т.к. в методе не удается пртписать условие пустого списка
        list.sortedAdd(2); list.sortedAdd(1);
        list.sortedAdd(5); list.sortedAdd(3);
        list.sortedAdd(4);
